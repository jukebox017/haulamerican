...
export default function EstimateTool() {
    ...
    const updatedServiceTypes = serviceTypes.map((serviceType) => {
      if (serviceType.id === id) {
        if (!serviceType.selected) {
          ...
          serviceType.amount = 1;
        }

        if (serviceType.selected && serviceType.amount === 0) {
  ...
  const adjustAmount = (id: string, newAmount: number) => {
    const updatedServiceTypes = serviceTypes.map((serviceType) => {
      if (serviceType.id === id) {
        if (newAmount > 5) newAmount = 5;
        serviceType.amount = newAmount;
      }
  ...
  return (
    <div id="estimate-container">
        ...
        {fiveOrLess === 'true' && (
          ...
          {serviceTypes.map((serviceType) => (
            <div className="service-type" key={serviceType.id}>
              <button
                type="button"
                className={`service-button ${serviceType.selected ? 'selected' : ''}`}
              ...
              >
                ...
                { serviceType.selected && (
                  <div className="amount-controls">
                    <div className="control" onClick={() => adjustAmount(serviceType.id, --serviceType.amount)}>-</div>
                    <div>{ serviceType.amount } </div>
                    <div className="control" onClick={() => adjustAmount(serviceType.id, ++serviceType.amount)}>+</div>
                  </div>
                )}
              </button>
      ...
      <div id="estimate">
        <h2>Your Estimate:</h2>
        {fiveOrLess == null || fiveOrLess == 'true' && (
          <>
            <h1>$0</h1>
        ...
        {(fiveOrLess != null && fiveOrLess == 'false') && (
          <p>Contact us at 719-266-2620 for your estimate!</p>
        )}
      </div>
    </div>
  )
